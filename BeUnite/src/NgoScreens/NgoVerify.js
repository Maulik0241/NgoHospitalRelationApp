...
export default function NgoVerify({navigation,route}) {
  ...
  const[actualCode,setActualCode] = useState(null);
    ...
    useEffect(()=>{
        setActualCode(ngoData[0]?.VerificationCode);
    },[]);
    ...
    const verify = async()=>{
        if(verificationCode ==''){
            ...
            const fdata = {
                orgname:ngoData[0]?.orgname,
                contactname:ngoData[0]?.contactname,
                contactemail:ngoData[0]?.contactemail,
                contactphone:ngoData[0]?.contactphone,
                address:ngoData[0]?.address,
                password:ngoData[0]?.password,
            }
            const endpoint = `http://10.0.2.2:3000/ngo/signup`;
            try{
