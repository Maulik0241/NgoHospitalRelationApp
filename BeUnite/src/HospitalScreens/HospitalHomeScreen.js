...
export default function HospitalHome({navigation}) {
  ...
  const fetchNGOList = async () => {
    const endpoint = `http://10.0.2.2:3000/ngo-list`;
    try {
  ...
  ngo.orgname.toLowerCase().includes(searchQuery.toLowerCase()),
);
  ...
  <ScrollView contentContainerStyle={{
    ...
    {filteredNGOList.map((ngo,index) => (
      <List.Item
        key={index} 
        onPress={() => {
          navigation.navigate('ShowMoreHospital', { ngoData: ngo });
        }}
        style={styles.item}
        title={
          <View style={styles.profile}>
          <View style={styles.profileImage}>
          <Image source={require('../../assets/images/ngoimage.jpg')} />
          </View>
            <View style={styles.textContainer}>
              <Text style={styles.orgName}>Organization Name: {ngo.orgname}</Text>
            </View>
          </View>
        }
      />
